...
TERRAFORM_DIR="$PROJECT_ROOT/terraform/postgresql"
...
if [ ! -d "$TERRAFORM_DIR" ]
then
    echo "ERROR: PostgreSQL Terraform directory not found:"
    echo "$TERRAFORM_DIR"
    exit 1
fi
...
LINUX_RESOURCES=(
    "null_resource.postgresql_validate_linux"
...
then
    ...
    echo "====================================="
    echo "POSTGRESQL TERRAFORM STATE RESET SUCCESSFUL"
    echo "====================================="
    echo
...
then
    ...
    echo "No PostgreSQL Linux resources found in Terraform state."
    echo "Nothing to reset."
    echo
...
echo "PostgreSQL Linux Terraform resources cleanup validated successfully."
