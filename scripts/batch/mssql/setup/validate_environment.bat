...
call "%ROOT%\scripts\batch\mssql\setup\validate_python_requirements.bat"
if errorlevel 1 (
...
echo [4/5] Validating MSSQL Port...
call "%ROOT%\scripts\batch\mssql\setup\validate_port.bat"
if errorlevel 1 (
    echo ERROR: MSSQL PORT VALIDATION FAILED
    exit /b 1
)

echo [5/5] Validating MSSQL...
call "%ROOT%\scripts\batch\mssql\setup\validate_mssql.bat"
if errorlevel 1 (
