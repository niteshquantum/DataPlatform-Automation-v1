...
set "SCHEMA_STATUS=%PROJECT_ROOT%\metadata\mysql\schema_status.json"
...
python scripts\cdc\cdc_engine.py mysql

set "CDC_EXIT=%ERRORLEVEL%"

if "%CDC_EXIT%"=="0" (

    echo Data changes detected. Proceeding with incremental load.
...
if "%CDC_EXIT%"=="100" (
    ...
    echo No data changes detected. Skipping data load successfully.
